...
<style>
    ...
    [data-testid="metric-container"] {
        background-color: #111827; border: 1px solid #1a2234;
    ...
    .badge-high { background:#7f1d1d; color:#f87171; padding:4px 12px; border-radius:16px; font-weight:700; display:inline-block; }
    .badge-med  { background:#78350f; color:#fbbf24; padding:4px 12px; border-radius:16px; font-weight:700; display:inline-block; }
    .badge-low  { background:#065f46; color:#34d399; padding:4px 12px; border-radius:16px; font-weight:700; display:inline-block; }
    .info-box { background:#111827; border-left:3px solid #60a5fa; padding:12px 16px; border-radius:4px; margin:8px 0; }
    .action-box { background:#111827; border-left:3px solid #34d399; padding:10px 14px; border-radius:4px; margin:6px 0; font-size:14px; }
</style>
""", unsafe_allow_html=True)
...
def dark_fig(figsize=(12,5)):
    fig, ax = plt.subplots(figsize=figsize, facecolor='#0a0f1c')
    ax.set_facecolor('#111827')
    ax.tick_params(colors='#9aa0a6')
    for spine in ax.spines.values(): spine.set_edgecolor('#1a2234')
    return fig, ax
...
def dark_figs(nrows, ncols, figsize):
    fig, axes = plt.subplots(nrows, ncols, figsize=figsize, facecolor='#0a0f1c')
    for ax in np.array(axes).flatten():
        ax.set_facecolor('#111827')
        ax.tick_params(colors='#9aa0a6')
        for spine in ax.spines.values(): spine.set_edgecolor('#1a2234')
...
if page == "🏠 Visão Geral":
    ...
    with col1:
        ...
        colors_p = [PALETTE[5] if v>20 else PALETTE[0] for v in cp.values]
        ax.bar(cp.index, cp.values, color=colors_p, alpha=0.85)
        ax.axhline(churn_rt, color='white', linestyle='--', alpha=0.5, label=f'Média {churn_rt:.1f}%')
        ax.set_ylabel('%', color='#9aa0a6')
        ax.legend(facecolor='#111827', labelcolor='#e8eaed')
        for i,v in enumerate(cp.values):
            ax.text(i, v+0.3, f'{v:.1f}%', ha='center', color='#e8eaed', fontsize=10)
        plt.tight_layout(); st.pyplot(fig); plt.close()
    ...
    with col2:
        ...
        colors_r = [PALETTE[3],PALETTE[4],PALETTE[1],PALETTE[5]]
        ax.bar(rc.index, rc.values, color=colors_r, alpha=0.85)
        ax.set_ylabel('Clientes', color='#9aa0a6')
        for i,v in enumerate(rc.values):
            ax.text(i, v+30, f'{v:,}', ha='center', color='#e8eaed', fontsize=10)
        plt.tight_layout(); st.pyplot(fig); plt.close()
    ...
    with col3:
        ...
        ax.plot(cp2.index, cp2.values, marker='o', color=PALETTE[0], linewidth=2.5, markersize=8)
        ax.fill_between(cp2.index, cp2.values, alpha=0.15, color=PALETTE[0])
        ax.set_xlabel('Produtos', color='#9aa0a6')
        ax.set_ylabel('%', color='#9aa0a6')
        plt.tight_layout(); st.pyplot(fig); plt.close()
...
# ── PAGE 2: RFM e Clusters ──────────────────────────────────
    ...
    with col1:
        ...
        ax2.set_ylabel('Churn Rate (%)', color=PALETTE[5])
        ax2.tick_params(colors=PALETTE[5])
        ax2.legend(facecolor='#111827', labelcolor='#e8eaed')
        for spine in ax2.spines.values(): spine.set_edgecolor('#1a2234')
        ax2.set_facecolor('#111827')
        plt.tight_layout(); st.pyplot(fig); plt.close()
    ...
    with col2:
        ...
        for churn_val, color, name in [(0,PALETTE[0],'Ativo'),(1,PALETTE[5],'Churn')]:
            vals = df[df['churned']==churn_val]['RFM_score']
            ax.hist(vals, bins=20, alpha=0.6, color=color, label=name, density=True)
        ax.set_xlabel('RFM Score', color='#9aa0a6')
        ax.legend(facecolor='#111827', labelcolor='#e8eaed')
        plt.tight_layout(); st.pyplot(fig); plt.close()
    ...
    with col3:
        ...
        colors_c = [PALETTE[5] if v>25 else PALETTE[4] if v>12 else PALETTE[0] for v in cc.values]
        ax.barh(cc.index, cc.values, color=colors_c, alpha=0.85)
        ax.set_xlabel('%', color='#9aa0a6')
        for i,v in enumerate(cc.values):
            ax.text(v+0.2, i, f'{v:.1f}%', va='center', color='#e8eaed', fontsize=9)
        plt.tight_layout(); st.pyplot(fig); plt.close()
    ...
    with col4:
        ...
        for i, cid in enumerate(sorted(cluster_ids)):
            ...
            ax.scatter(df[mask]['engagement_score'], df[mask]['RFM_score'],
                      color=PALETTE[i%len(PALETTE)], alpha=0.3, s=8,
                      label=cluster_names.get(cid, f'C{cid}'))
        ax.set_xlabel('Engagement Score', color='#9aa0a6')
        ax.set_ylabel('RFM Score', color='#9aa0a6')
        ax.legend(facecolor='#111827', labelcolor='#e8eaed', fontsize=8, markerscale=3)
        plt.tight_layout(); st.pyplot(fig); plt.close()
...
# ── PAGE 3: Score Individual ─────────────────────────────────
    ...
    with col_out:
        # Calcular features derivadas
        r_score = int(pd.qcut([last_txn_v], 5, labels=[5,4,3,2,1], duplicates='drop')[0]) if last_txn_v > 0 else 5
        f_score = min(5, max(1, int(monthly_txn_v / 12) + 1))
        m_val   = balance_v + cc_spend_v
        m_score = min(5, max(1, int(m_val / 15000) + 1))
        rfm_s   = r_score + f_score + m_score
        ...
        with tab1:
            ...
            ax.set_title('Fatores que contribuem para o risco', color='#e8eaed')
            ax.set_xlabel('SHAP value (+ aumenta risco)', color='#9aa0a6')
            plt.tight_layout(); st.pyplot(fig); plt.close()
        ...
        with tab2:
            ...
            ax.bar(labels, scores, color=colors_rfm, alpha=0.85)
            ax.set_ylim(0, 5.5)
            ax.set_ylabel('Score', color='#9aa0a6')
            ax.set_title('Scores RFM do Cliente', color='#e8eaed')
            for i,v in enumerate(scores):
                ax.text(i, v+0.1, f'{v}/5', ha='center', color='#e8eaed', fontsize=12, fontweight='bold')
            plt.tight_layout(); st.pyplot(fig); plt.close()
...
# ── PAGE 4: Plano de Intervenção ────────────────────────────
    ...
    ax.set_xlabel('Churn Rate (%)', color='#9aa0a6')
    ax.set_ylabel('Clientes em Risco (Alto + Crítico)', color='#9aa0a6')
    ax.set_title('Priorização: Churn Rate vs Volume em Risco', color='#e8eaed')
    plt.tight_layout(); st.pyplot(fig); plt.close()
